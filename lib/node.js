...
var getterOfDataFromExternalApi = require('./apis/end_to_end');
...
exports.getWeather = function (query) {
	...
	ok = ok.then(function (dataFromDb) {
		...
		ok = ok.then(function (data) {
			...
			if (data.lock) {
				var okRelease = model.release_lock(data.city);
...
function completeIfNeeded (dataFromDb) {
		...
		if (typeof data.weather[apiName][requiredWeatherParameters[0]] === 'undefined') {
			...
			return ok.then(
				...
				function (err) {
					...
					logger.error({err : err, api : apiName}, 'Error occured while getting data from api');
				}
			);
		}
	});
	...
	ok = ok.then(function () {
		logger.info({dataFromDb : dataFromDb, dataAfterCompletion : data}, 'Data change.');
	});
